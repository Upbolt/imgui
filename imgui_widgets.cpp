...
void ImGui::LabelTextClipped(const char* label) {
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return;
    ...
    ImVec2 size = CalcItemSize(ImVec2{}, label_size.x + style.FramePadding.x * 2.0f, label_size.y + style.FramePadding.y * 2.0f);
...
// Add a label+text combo aligned to other label+value widgets
void ImGui::LabelTextBackground(const char* label)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return;
